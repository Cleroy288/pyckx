...
pub async fn create_custom_question_handler(
    ...
    let level = metadata.parse_level().map_err(|e| crate::shared::AppError::validation("level", e))?;
    ...
    let qcm_set = app.intello_service.generate_ai_qcm(&user_id, service_input).await?;
...
pub async fn create_open_question_handler(
    ...
    let source_content: String = documents.iter()
    ...
    let question_set = app.intello_service.generate_ai_open_questions(&user_id, service_input, source_content).await?;

    // Build response
    let question_responses: Vec<OpenQuestionResponse> = question_set.questions.iter()
        .map(OpenQuestionResponse::from)
        .collect();

    Ok(HttpResponse::Created().json(CreateOpenQuestionResponse {
        success: true,
        message: format!("Open question set created with {} questions", question_set.questions.len()),
...
pub async fn create_flashcard_handler(
    ...
    // Call service directly
    let flashcard_set = app.intello_service.generate_ai_flashcards(&user_id, service_input).await?;
