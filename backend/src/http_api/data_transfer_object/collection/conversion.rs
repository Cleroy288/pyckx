...
use super::response::{
    CollectionItemsResponse, CollectionListResponse, CollectionResponse, CollectionSuccessResponse,
...
impl CollectionItemsResponse {
    pub fn from_dvds(dvds: Vec<Dvd>) -> Self {
        let count = dvds.len();
        let items_json =
            serde_json::to_value(dvds.into_iter().map(DvdResponse::from).collect::<Vec<_>>())
                .unwrap_or_default();
