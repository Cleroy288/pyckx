...
use crate::domain::intello::DocumentType;
use crate::error::{AppError, AppResult};
use crate::shared::{extract_pdf, extract_pptx, extract_txt, extract_word};
        ...
        match field_name {
            "metadata" => {
                ...
                while let Some(chunk) = field.next().await {
                    let chunk = chunk.map_err(|e| AppError::validation("metadata", e.to_string()))?;
...
pub fn get_document_type(filename: &str) -> AppResult<DocumentType> {
    if filename.ends_with(".pdf") {
        ...
        Err(AppError::validation(
            "files",
            format!("Unsupported file type: {}. Supported: .txt, .pdf, .docx, .pptx", filename),
        ))
    }
}
...
pub async fn parse_multipart_files_only(
    ...
    if documents.is_empty() {
        return Err(AppError::validation("files", "No files provided in the upload"));
    }
