...
use crate::infrastructure::{AppRepository, CreateApp, SupabaseAppRepository};
...
async fn test_app_crud_lifecycle() {
    ...
    let repo = SupabaseAppRepository::new(&config);
    ...
    // == 4. UPDATE APP ==
    println!("\n=== Step 4: Update App ===");
    let update = crate::infrastructure::UpdateApp::new().with_description("Updated description");
    let updated = repo
...
async fn test_duplicate_app_creation_fails() {
    ...
    match result.unwrap_err() {
        AppError::AppAlreadyExists(name) => {
