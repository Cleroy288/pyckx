...
async fn test_create_qcmset_rejects_empty_name() {
    ...
    match err {
        crate::error::IntelloError::ValidationFailed { field, .. } => {
...
async fn test_create_qcmset_rejects_whitespace_name() {
    ...
    let result = service.create_qcm_set(qcmset).await;
    assert!(result.is_err(), "Should reject QCM set with whitespace-only name");
...
async fn test_create_qcmset_rejects_empty_description() {
    ...
    let result = service.create_qcm_set(qcmset).await;
    assert!(result.is_err(), "Should reject QCM set with empty description");

    let err = result.unwrap_err();
    match err {
        crate::error::IntelloError::ValidationFailed { field, .. } => {
            assert_eq!(field, "description", "Error should be on 'description' field");
        }
        _ => panic!("Expected ValidationFailed error, got {:?}", err),
    }
}
...
async fn test_create_qcmset_rejects_question_with_wrong_answer_count() {
    ...
    let result = service.create_qcm_set(qcmset).await;
    assert!(result.is_err(), "Should reject QCM set with wrong answer count");

    let err = result.unwrap_err();
    match err {
        crate::error::IntelloError::ValidationFailed { field, message } => {
            assert_eq!(field, "questions", "Error should be on 'questions' field");
            assert!(message.contains("3 wrong answers"), "Error message should mention 3 wrong answers");
        }
        _ => panic!("Expected ValidationFailed error, got {:?}", err),
    }
}
...
async fn test_create_qcmset_rejects_question_with_empty_text() {
    ...
    let result = service.create_qcm_set(qcmset).await;
    assert!(result.is_err(), "Should reject QCM set with empty question text");

    let err = result.unwrap_err();
    match err {
        crate::error::IntelloError::ValidationFailed { field, message } => {
            assert_eq!(field, "questions", "Error should be on 'questions' field");
            assert!(message.contains("question text"), "Error message should mention question text");
        }
        _ => panic!("Expected ValidationFailed error, got {:?}", err),
    }
}
...
async fn test_create_qcmset_rejects_question_with_empty_right_answer() {
    ...
    let result = service.create_qcm_set(qcmset).await;
    assert!(result.is_err(), "Should reject QCM set with empty right_answer");

    let err = result.unwrap_err();
    match err {
        crate::error::IntelloError::ValidationFailed { field, message } => {
            assert_eq!(field, "questions", "Error should be on 'questions' field");
            assert!(message.contains("right answer"), "Error message should mention right answer");
        }
        _ => panic!("Expected ValidationFailed error, got {:?}", err),
    }
}
