...
use super::types::{AnswerGrade, CheckAnswersInput, GenerateContentInput, GradingResult, IntelloService};
use crate::domain::intello::OpenQuestionSet;
use crate::error::IntelloError;
use crate::shared::{AnswerToGrade, OpenQuestionPromptInput, VerificationPromptInput};
use tracing::{info, instrument};

impl IntelloService {
    /// Get all open question sets for a user
    #[instrument(skip(self), fields(user_id = %user_id))]
    pub async fn get_user_open_question_sets(&self, user_id: &str) -> Result<Vec<OpenQuestionSet>, IntelloError> {
    ...
    pub async fn generate_ai_open_questions(
        ...
        info!(num_questions = input.num_questions, model = ?input.model, "Generating AI open questions");
        ...
        let questions = self.openrouter_service.generate_open_questions(&prompt_input, input.model.as_deref()).await?;
    ...
    pub async fn check_open_question_answers(
        ...
        for user_answer in &input.answers {
            if let Some(question) = set.questions.iter().find(|q| q.id == user_answer.question_id) {
        ...
        if answers_to_grade.is_empty() {
            return Err(IntelloError::validation("answers", "No valid answers to grade"));
        }
        ...
        let results: Vec<GradingResult> = grades.into_iter().map(|g| {
            GradingResult {
