...
pub fn parse_generated_synthesis(ai_response: &str) -> Result<ParsedSynthesis, IntelloError> {
    ...
    let synthesis: ParsedSynthesis = serde_json::from_str(&sanitized).map_err(|e| {
        IntelloError::validation(
            "synthesis_json",
            format!("Failed to parse synthesis JSON: {}", e),
        )
    })?;
    ...
    for (idx, question) in synthesis.final_qcm.questions.iter().enumerate() {
        ...
        if question.wrong_answers.len() != 3 {
            return Err(IntelloError::validation(
                "wrong_answers",
                format!("Question {} must have exactly 3 wrong answers", idx + 1),
            ));
        }
    }
