...
impl IntelloService {
    ...
    pub async fn get_qcm_set(&self, set_id: &str, user_id: &str) -> Result<Option<QcmSet>, IntelloError> {
        self.validate_user_id(user_id)?;
    ...
    pub async fn update_qcm_set(&self, qcm_set: QcmSet) -> Result<bool, IntelloError> {
        ...
        // Verify ownership
        let existing = self.qcm_repo.find_by_id(&qcm_set.id, &qcm_set.user_id).await?;
        if existing.is_none() {
    ...
    pub async fn generate_ai_qcm(
        ...
        info!(num_questions = input.num_questions, model = ?input.model, "Generating AI QCM questions");

        // Build CustomQuestion for OpenRouter
        let custom_question = self.build_custom_question(user_id, &input);

        // Generate questions via AI
        let questions = self.openrouter_service.generate_qcm(&custom_question, input.model.as_deref()).await?;
