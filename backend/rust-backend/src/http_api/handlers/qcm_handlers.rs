...
use crate::http_api::data_transfer_object::{
    CreateQcmSetRequest, QcmSetListResponse,
    QcmSetResponse, QcmSuccessResponse,
    UpdateQcmSetRequest,
};
use crate::infra::user::get_user_id_from_session;
use crate::services::{
    CreateQcmInput, QcmQuestion, QcmSet,
...
/// POST /api/study/qcm - Create a new manual QCM set
#[post("/qcm")]
...
pub async fn create_qcmset_handler(
    ...
    let qcmset = QcmSet::from_input(&user_id, input);
    let created = app
        .study_service
        .create_qcm_set(qcmset)
        .await?;
    Ok(HttpResponse::Created().json(QcmSuccessResponse::created(&created)))
}

/// GET /api/study/qcm - Get all user's QCM sets
#[get("/qcm")]
...
pub async fn get_user_qcmsets_handler(
    ...
    let user_id = get_user_id_from_session(&app, &req).await?;
    let sets = app.study_service.get_user_qcm_sets(&user_id).await?;
    Ok(HttpResponse::Ok().json(QcmSetListResponse::from_sets(sets)))
}

/// GET /api/study/qcm/{id} - Get a specific QCM set
#[get("/qcm/{set_id}")]
...
pub async fn get_qcmset_handler(
    ...
    match app.study_service.get_qcm_set(&set_id, &user_id).await? {
...
/// PUT /api/study/qcm/{id} - Update a QCM set
#[put("/qcm/{set_id}")]
...
/// DELETE /api/study/qcm/{id} - Delete a QCM set
#[delete("/qcm/{set_id}")]
#[instrument(skip(app, req), fields(set_id = %path.as_str()))]
