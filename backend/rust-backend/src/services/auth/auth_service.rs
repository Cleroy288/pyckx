...
impl AuthService {
    ...
    pub async fn login(&self, email: &str, password: &str) -> AppResult<User> {
        ...
        self.sessions.create_session(user.clone()).await;
    ...
    pub async fn logout(&self, session_id: &str) -> bool {
        // Remove from Redis and get user data (contains access_token)
        let user = self.sessions.delete_session(session_id).await;
