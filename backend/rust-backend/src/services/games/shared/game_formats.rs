//! Game output format definitions
...
/// Look up a game format by ID
pub fn get_game_format(
    game_id: &str,
) -> Option<&'static GameOutputFormat> {
    GAME_FORMATS.iter().find(|f| f.game_id == game_id)
}

/// Look up a game format, falling back to a default
pub fn get_game_format_or_default(
    game_id: &str,
) -> &'static GameOutputFormat {
    get_game_format(game_id)
        .unwrap_or(&GAME_FORMATS[0])
}
...
mod tests {
    ...
    #[test]
    fn test_get_game_format_qcm_returns_some() {
        // arrange / act
        let result = get_game_format("qcm");

        // assert
        assert_eq!(result.unwrap().game_id, "qcm");
    }

    #[test]
    fn test_get_game_format_unknown_returns_none() {
        // arrange / act
        let result = get_game_format("unknown_game");
    ...
    #[test]
    fn test_get_game_format_all_types_exist() {
        // arrange
        let ids = vec![
            "qcm", "open_question", "flashcard",
            "true_false", "keywords", "order_phrase",
            "fill_blank",
        ];

        for id in ids {
            // act
            let result = get_game_format(id);

            // assert
            let fmt = result.unwrap_or_else(
                || panic!("missing format: {}", id),
            );
            assert_eq!(fmt.game_id, id);
        }
    }

    #[test]
    fn test_get_game_format_or_default_unknown_returns_qcm()
    {
        // arrange / act
        let result =
            get_game_format_or_default("nonexistent");

        // assert
        assert_eq!(result.game_id, "qcm");
    }

    #[test]
    fn test_get_game_format_or_default_known_returns_match()
    {
        // arrange / act
        let result =
            get_game_format_or_default("flashcard");

        // assert
        assert_eq!(result.game_id, "flashcard");
    }
}
