...
pub fn dispatch_block_parser(
    block: &Value,
) -> Result<Option<ContentBlock>, StudyError> {
...
/// Parse title block
fn parse_title(value: &Value) -> Result<ContentBlock, StudyError> {
    let content = value
        .get("content")
        .and_then(|v| v.as_str())
        .ok_or_else(|| StudyError::validation("title", "Missing content"))?
        .to_string();
...
fn parse_subtitle(value: &Value) -> Result<ContentBlock, StudyError> {
    let content = value
        .get("content")
        .and_then(|v| v.as_str())
        .ok_or_else(|| StudyError::validation("subtitle", "Missing content"))?
        .to_string();
...
fn parse_text_section(value: &Value) -> Result<ContentBlock, StudyError> {
    // Use serde to trigger deserialize_string_or_vec helper
    let block: ContentBlock =
        serde_json::from_value(value.clone()).map_err(|err| {
            StudyError::validation("text_section", err.to_string())
        })?;
...
fn parse_schema(value: &Value) -> Result<ContentBlock, StudyError> {
    let block: ContentBlock = serde_json::from_value(value.clone())
        .map_err(|err| StudyError::validation("schema", err.to_string()))?;
...
/// Parse QCM set block
fn parse_qcm_set(value: &Value) -> Result<ContentBlock, StudyError> {
    ...
    let data: QcmSetPayload = serde_json::from_value(data_val.clone())
        .map_err(|err| StudyError::validation("qcm_set", err.to_string()))?;

    Ok(ContentBlock::QcmSet { data })
}

/// Parse flashcard set block
fn parse_flashcard_set(value: &Value) -> Result<ContentBlock, StudyError> {
    let data_val = value.get("data").unwrap_or(value);

    let data: FlashcardSetPayload = serde_json::from_value(data_val.clone())
        .map_err(|err| {
            StudyError::validation("flashcard_set", err.to_string())
        })?;
...
fn parse_true_false_set(value: &Value) -> Result<ContentBlock, StudyError> {
    let data_val = value.get("data").unwrap_or(value);

    let data: TrueFalseSetPayload = serde_json::from_value(data_val.clone())
        .map_err(|err| {
            StudyError::validation("true_false_set", err.to_string())
        })?;
