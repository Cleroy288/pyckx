...
use crate::http_api::data_transfer_object::course::CourseModule;
...
pub async fn generate_learn_content(
    ...
    let response = service
        ...
        .await
        .map_err(|err| StudyError::external("OpenRouter", err.to_string()))?;
...
pub async fn orchestrate_learn_mode_generation(
    ...
    let session = repo.get(session_id).await.map_err(|err| {
        StudyError::validation("session_lookup", err.to_string())
    })?;
