//! True/False E2E tests — list + create via HTTP.
...
/// Build a multipart body for true/false creation.
/// Returns (content_type_header, body_bytes).
fn build_true_false_multipart() -> (String, Vec<u8>) {
    let boundary = "----TestBoundary";
    let metadata = serde_json::json!({
        "name": "E2E True False Set",
        "description": "Created by E2E test",
        "instructions": "Generate true/false statements",
        "language": "en",
        "level": "easy",
        "subjects": ["test"],
        "num_questions": 2
...
async fn test_list_true_false_returns_200() {
    // arrange
...
#[actix_rt::test]
#[ignore = "requires Supabase connection"]
async fn test_list_true_false_has_sets_array() {
    // arrange
    let srv = spawn_app().await;
    let session = login(&srv).await;
    let cookie = session_cookie_value(&session);
...
/// POST true/false + verify created set appears in list.
/// NOTE: no DELETE endpoint exists for true/false —
/// test data leaks into the test database.
#[actix_rt::test]
#[ignore = "requires Supabase connection"]
async fn test_create_true_false_set_and_list_returns_it() {
    // arrange
    let srv = spawn_app().await;
    let session = login(&srv).await;
    let ck = session_cookie_value(&session);
    let (ct, body) = build_true_false_multipart();

    // act — create
    let mut resp = srv
        .post("/api/study/true-false")
