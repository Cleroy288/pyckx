//! Stub repositories and factory for StudyService integration tests
...
use LAPP::services::ai_usage::ai_usage_domain::{
    AiUsageLog, CreateAiUsageLog,
};
use LAPP::services::course::domain::{
    Course, ResourceSummary, UserResource,
};
use LAPP::services::error_domain::StudyError;
use LAPP::services::games::open_question::open_question_cache_service::OpenQuestionCache;
use LAPP::services::{
    FillBlankSet, FlashcardSet, StudyRepositories, StudyService,
...
impl LAPP::infra::StudySessionRepository for NoopStudySession {
    async fn create(
        &self,
        s: &LAPP::services::study_session::study_session_domain::StudySession,
    ) -> Result<
        LAPP::services::study_session::study_session_domain::StudySession,
        AppError,
    > {
        Ok(s.clone())
    }
    async fn list_by_course(
        ...
        Vec<LAPP::services::study_session::study_session_domain::StudySession>,
        AppError,
    > {
        Ok(vec![])
    }
...
/// Build an StudyService with in-memory stubs
pub fn make_study_service() -> StudyService {
    ...
    StudyService::builder()
        ...
        .build()
        .expect("test StudyService setup")
}
