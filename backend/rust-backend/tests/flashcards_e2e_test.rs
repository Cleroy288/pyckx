//! Flashcard E2E tests — list + create via HTTP.
...
/// Build a multipart body for flashcard creation.
/// Returns (content_type_header, body_bytes).
fn build_flashcard_multipart() -> (String, Vec<u8>) {
    let boundary = "----TestBoundary";
    let metadata = serde_json::json!({
        "name": "E2E Flashcard Set",
        "description": "Created by E2E test",
        "instructions": "Generate flashcards",
        "language": "en",
        "level": "easy",
        "subjects": ["test"],
        "num_questions": 2
...
async fn test_list_flashcards_returns_200() {
    // arrange
...
#[actix_rt::test]
#[ignore = "requires Supabase connection"]
async fn test_list_flashcards_has_sets_array() {
    // arrange
    let srv = spawn_app().await;
    let session = login(&srv).await;
    let cookie = session_cookie_value(&session);
...
/// POST flashcards + verify created set appears in list.
/// NOTE: no DELETE endpoint exists for flashcards —
/// test data leaks into the test database.
#[actix_rt::test]
#[ignore = "requires Supabase connection"]
async fn test_create_flashcard_set_and_list_returns_it() {
    // arrange
    let srv = spawn_app().await;
    let session = login(&srv).await;
    let ck = session_cookie_value(&session);
    let (ct, body) = build_flashcard_multipart();
